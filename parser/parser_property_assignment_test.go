...
Rectangle {
	...
	if propertyAssignmentCount != 3 {
		t.Errorf("Incorrect property assignments count, expected '3', take %d", propertyAssignmentCount)
	}
	...
	if prop3.Property[0] != "color" {
		t.Errorf("Incorrect second property, expected 'color', take %s", prop3.Property[0])
	}
	arg3, ok := prop3.Expression.(*literals.StringLiteral)
	if ok && *arg3 != "red" {
		t.Errorf("Incorrect second property value, expected 'red', take %s", *arg3)
	} else if !ok {
		t.Errorf("Incorrect second property type, expected StringLiteral")
	}
}
